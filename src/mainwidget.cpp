...
MainWidget::MainWidget(QWidget *parent)
  ...
  // 处理功能选项
  connect(ui->header_funcs, &QComboBox::currentIndexChanged,
          [&](int index) { ui->body_show->setCurrentIndex(index); });

  // 处理底部长度信息事件
  ui->rtx_len->installEventFilter(this);
}

MainWidget::~MainWidget() { delete ui; }

void MainWidget::exit() { QApplication::quit(); }
...
void MainWidget::minimize() { showMinimized(); }

void MainWidget::fixed() {
  onTop = !onTop;
#ifdef Q_OS_WIN
  HWND h = (HWND)winId();
  SetWindowPos(h, onTop ? HWND_TOPMOST : HWND_NOTOPMOST, 0, 0, 0, 0,
...
bool MainWidget::eventFilter(QObject *obj, QEvent *event) {
  ...
  } else if (obj == ui->rtx_len && event->type() == QEvent::MouseButtonPress) {
    // 处理底部接收发送长度事件
...
void MainWidget::setBgDark(bool dark) {
  static const auto isWindows11 = []() -> bool {
    ...
    return v.type() == QOperatingSystemVersionBase::Windows &&
           v.majorVersion() == 10 && v.microVersion() >= 22000; // Windows 11
  };
...
void MainWidget::addtx(uint32_t len) {
  txcount += len;
  ui->tx_len->setText(QString::number(txcount));
}
void MainWidget::resetrx() {
  rxcount = 0;
  ui->rx_len->setText(QString::number(rxcount));
}
void MainWidget::resettx() {
  txcount = 0;
  ui->tx_len->setText(QString::number(txcount));
}
void MainWidget::resetrtx() {
  rxcount = 0;
  txcount = 0;
  ui->rx_len->setText(QString::number(rxcount));
  ui->tx_len->setText(QString::number(txcount));
