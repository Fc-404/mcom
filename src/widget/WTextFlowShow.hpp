...
public:
  WTextFlowShow(QWidget *parent = nullptr) : QPlainTextEdit(parent) {
    setReadOnly(true);
   ...
   */
  WTextFlowShow(QWidget *parent, bool useconfig) : QPlainTextEdit(parent) {
   ...
   * 获取是否显示十六进制序号
   */
  bool onHexNo() { return onhexno; }
  /**
   * 设置是否显示十六进制序号
   */
  void onHexNo(bool on) {
    onhexno = on;
    if (redraw)
      reshow();
  }
   ...
   * @param type 类型
   */
  void append(QByteArray data, QTime time = QTime::currentTime(),
              TextType type = Rx) {
